...
class reportIndvController {
      ...
      function CreateContent()
    {
        ...
        $reportArray = $sqlModel->GetReports();
            
			if (($_GET['edited']) == "yes")
			{
				 echo '<script language="javascript">';
				echo 'alert("Report successfully Edited")';
				 echo '</script>';
			}
			if (empty($_GET['role']))
                    {     
            ...
            <!-- /. PAGE INNER  -->";
					...
					else
					{
					 ...
					 $sqlModel = new sqlModel();
						...
						$ReportIndividiualArray = $sqlModel->GetReportDetails($report);
                ...
                <div class='row'>
								...
								foreach ($ReportIndividiualArray as $value )
						{
								if ( $first )
							{
									...
									<form action='model/updateReport.php' method='post'>
									...
									session_start();
									$changestatus = $_SESSION["changestatus"];
									
									if ($changestatus == "1")
									{
									$result = $result . "Report Status: <br><select name='ReportStatus' >
									...
									{
									$result = $result . "Report Status (readonly):<br> <input type='text' name='ReportStatus' value='$value->reportStatus' size='50' readonly><br>";	
									}
									$result = $result . "
